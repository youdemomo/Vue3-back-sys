...
2. 在el-table-column中，给prop传入的值默认用作用域插槽展示，所以品牌logo列插槽应做更改，变为img插槽。row为传入数组的项，$index为该项在数组中的下标
...
​	在error事件回调中，将isImgDie置为true，表示图片无法加载，使用v-else的本地图片
...
​	主体结构粘官网的 el-dialog 组件，其中包含一个form表单
...
​	它会根据校验结果返回状态不同的Promise，使用 await 即可拦截失败的校验
...
​	reference插槽为点击后触发气泡框的按钮，@confirm 为气泡框确认的回调
